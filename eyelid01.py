...
class eyelidTrial():
    def __init__(self,sweeplength):
        ...
        self.USonset = 0
        self.trialName = ""
        self.sweep =  np.zeros(sweeplength)
        self.keeper = 1
        self.CRamp = 0
        self.criterion = 0
        self.onset = 0
...
def basicInfo(file):
    ...
    if temp == "None": 
        ...
        vers = "newBT" 
    if vers != "oldBT":
        temp = np.fromfile(file,dtype=float)
    else: 
...
def read_dat(datFile, version, numTrials, Data):
    if version != "oldBT":
        temp = np.fromfile(datFile,dtype=float)
        s = len(temp)
...
def waterfall(data,version, sweepLength,drawto, startTrial,stopTrial):
    ...
    for i in range(stopTrial-1,startTrial-2,-1):
        if version != "oldBT": trace = np.add(data[i].sweep,(sweepnum*.3))
        xVals = range(xoffset,xend)
        ax.plot(xVals,trace[0:drawto],linewidth=2,color='gray')
        CSlength = int(data[i].CSduration)            
        if version == "cbm_sim": 
            CSlength = int(CSlength/2)
            ax.plot(xVals[200:200+CSlength],trace[200:200+CSlength],linewidth=2,color='black')
        if version == "newBT": 
            ax.plot(xVals[200:200+CSlength],trace[200:200+CSlength],linewidth=2,color='black')
        sweepnum = sweepnum - 1
...
def filterAndNormalize(data, version, numTrials, sweepLength, cal):
        if version == "cbm_sim":
    ...
    if version == 'cbm_sim':
        for trials in range(0,numTrials):
            ...
            if data[trials].USonset == 0:
                stopper = CSon + int(data[trials].CSduration/2) # format
            else:
                stopper = CSon + int((data[trials].USonset-200)/2) # format, most CRs are too late for this??
            ...
            if data[trials].CRamp > .3: # it's a CR, so do criterion and latency too
                ...
                for b in range(CSon,stopper):
                    if sweep[b]>.3:
                        data[trials].criterion = (b-200)*2 # ugh, format
                        break
                while sweep[b]>.001:
                    b = b-1
                    if b < 200:
